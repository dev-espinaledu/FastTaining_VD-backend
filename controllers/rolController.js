...
exports.crearRol = async (req, res) => {
    ...
    try {
        const { rows } = await pool.query("INSERT INTO roles (nombre) VALUES ($1) RETURNING *", [nombre]);
        console.log("Nombre recibido:", nombre);
        res.status(201).json({ message: "Rol creado", rol: rows[0] });
    } catch (error) {
...
exports.actualizarRol = async (req, res) => {
    ...
    try {
        const { rowCount } = await pool.query("UPDATE roles SET nombre = $1 WHERE id = $2", [nombre, id]);
