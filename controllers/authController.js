...
const login = async (req, res) => {
  try {
    const rol_id = Usuario.findByPk(rol_id);
    ...
    if (intentosFallidos[email] && intentosFallidos[email].intentos >= 5) {
      const tiempoBloqueo =
        (Date.now() - intentosFallidos[email].primerIntento) / 1000 / 60;
      if (tiempoBloqueo < 15) {
    ...
    const token = jwt.sign(
      { userId: user.id, role: rol },
      process.env.JWT_SECRET,
      { expiresIn: "1h" },
    );

    res.json({ message: "Login exitoso", token, role: rol });
  } catch (error) {
